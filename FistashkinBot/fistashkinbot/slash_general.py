...
from utils import constant, enums, main, links, faq, database, paginator
...
class General(commands.Cog, name="Основное"):
    ...
    async def autocomplete_faq(
        ...
        return list(
            filter(
                ...
                [i["question"] for i in faq.Faq.FAQ],
            )
        )
    ...
    async def faq(
        ...
        if question in [i["question"] for i in faq.Faq.FAQ]:
            await inter.response.defer(ephemeral=False)
            embed = disnake.Embed(
                description=[
                    i["answer"] for i in faq.Faq.FAQ if i["question"] == question
                ][0],
