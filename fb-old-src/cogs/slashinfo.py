...
class CommandInfoBySlash(commands.Cog):
    ...
    async def userinfo(self, inter, member: disnake.Member = None):
        if member == None:
            ...
            emb.set_footer(
                text=f"ID: {inter.author.id}", icon_url=inter.author.display_avatar.url
            )
            emb.set_author(
    ...
    @commands.slash_command(
        name="роли", description="Показывает все роли сервера.", dm_permission=False
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    @commands.slash_command(
        name="инфо", description="Показывает интересную информацию о боте."
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
...
def setup(bot):
    bot.add_cog(CommandInfoBySlash(bot))
