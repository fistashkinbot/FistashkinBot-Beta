...
class OwnerInfo(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.content.lower() == ("developer_cmd_reset_table"):
            if message.author.id == self.main.DEVELOPER_ID:
                print("Начался процесс обнуления таблиц")
                message = await message.channel.send(
                    f"{message.author.mention}, начался процесс обнуления следующих таблиц: **__level__**, **__xp__**, **__bio__** по **__{len(self.bot.guilds)}__** серверам! Это займёт некоторое время! Ожидайте..."
                )
                for guild in self.bot.guilds:
                    for member in guild.members:
                        await self.db.update_member(
                            "UPDATE users SET balance = ? WHERE member_id = ? AND guild_id = ?",
                            [50000, member.id, member.guild.id],
                        )
                        await self.db.update_member(
                ...
                await message.edit(
                    f"{message.author.mention}, таблицы **__level__**, **__xp__** по **__{len(self.bot.guilds)}__** серверам были сброшены!",
                    delete_after=15.0,
                )
                print("Процесс обнуления таблиц завершен")
            else:
                await message.reply(
        ...
        if message.content.lower() == ("developer_reset_handled_cmds"):
            if message.author.id == self.main.DEVELOPER_ID:
                await self.db.insert_command_count_debug(amount=9679)
            else:
                await message.reply(
                    f"{message.author.mention}, ты не являешься владельцем бота, поэтому команда не доступна!",
                    delete_after=15.0,
