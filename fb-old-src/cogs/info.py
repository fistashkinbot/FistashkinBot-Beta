...
class CommandInfo(commands.Cog):
    ...
    async def userinfo(self, inter, member: disnake.Member = None):
        async with inter.typing():
            if member == None:
                ...
                emb.set_footer(
                    text=f"ID: {member.id}", icon_url=member.display_avatar.url
                )
                emb.set_author(
                    name=f"{inter.author.name}",
                    icon_url=inter.author.display_avatar.url,
    ...
    @commands.command(
        name="сервер", description="Показывает информацию о сервере.", usage="сервер"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.guild_only()
    async def serverinfo(self, inter):
        async with inter.typing():
            ...
            embed.add_field(
                name="👑 Создатель:", value=f"{inter.guild.owner.mention}", inline=True
            )
            embed.add_field(
                name="📆 Создан:",
                value=f"<t:{round(inter.guild.created_at.timestamp())}:D>\n<t:{round(inter.guild.created_at.timestamp())}:R>",
                inline=True,
            )
            embed.add_field(
                name="👥 Участники:",
                value=f"**{inter.guild.member_count}** Участник(а/ов)",
                inline=True,
            )
            embed.add_field(
                name="💬 Каналы:",
                value=f"🔮 Категорий: **{len(inter.guild.categories)}** | 💬 Текстовых: **{len(inter.guild.text_channels)}** | 🎵 Голосовых: **{len(inter.guild.voice_channels)}**",
                inline=True,
            )
            embed.set_thumbnail(url=inter.guild.icon.url)
            embed.set_footer(
                text=f"ID: {inter.guild.id}", icon_url=inter.guild.icon.url
            )
            embed.set_author(
                name=f"{inter.author.name}", icon_url=inter.author.display_avatar.url
            )
            await asyncio.sleep(0.5)
    ...
    async def avatar(self, inter, member: disnake.Member = None):
        async with inter.typing():
            ...
            embed.set_footer(
                text=f"{footerbyriverya4life}", icon_url=inter.author.display_avatar.url
            )
            embed.set_author(
                name=f"{inter.author.name}", icon_url=inter.author.display_avatar.url
            )
    ...
    async def roles(self, inter):
        async with inter.typing():
            ...
            embed.set_footer(
                text=f"Запросил {inter.author.display_name}",
                icon_url=inter.author.display_avatar.url,
            )
            await asyncio.sleep(0.5)
    ...
    @commands.command(
        name="инфо",
        description="Показывает интересную информацию о боте.",
        usage="инфо",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def stats(self, inter):
        async with inter.typing():
            ...
            emb.add_field(
                name="👾 Сборка:", value=f"{riverya4lifebotversion}", inline=True
            )
            emb.add_field(
                name="🔮 Мой разработчик:",
                value=f"<:riverya4life:1044666044599504906> Riverya4life#9515",
                inline=True,
            )
            emb.add_field(
                name="🎊 Дата создания:",
            ...
            emb.add_field(
                ...
                value=f"[💖 Github](https://github.com/riverya4life) | [🎥 YouTube](https://www.youtube.com/@riverya4lifehomie) | [📱 TikTok](https://www.tiktok.com/@riverya4life)",
                inline=False,
            )
