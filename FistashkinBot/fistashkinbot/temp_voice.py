...
class TempVoice(commands.Cog):
    ...
    async def on_voice_state_update(
        ...
        if after.channel is not None:
            trigger_info = await self.db.get_voice_channel_trigger(member.guild.id, after.channel.id)
            if trigger_info and after.channel.id == trigger_info['channel_id']:
                ...
                private_channel = await guild.create_voice_channel(
                    f'Приват {member.display_name}',  # название канала
                ...
                await private_channel.set_permissions(
                    member, connect=False, mute_members=True, move_members=True, manage_channels=True
                )  # установить права на канал его создателю
                await member.move_to(private_channel)  # переместить пользователя в этот канал
